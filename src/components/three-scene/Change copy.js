  ...
  radialMove() {
    Object.values(this.sheDengMap).map((sd) => {
      Utils.anima(
        ...
        (data) => {
          sd.model.rotation.x = data.R.x;
          sd.model.rotation.y = data.R.y;
          sd.model.rotation.z = data.R.z;
        },
        () => { }
      );
    })
...
class Events {
  constructor() {
    ...
    t.runScene.optionsEx.cb.events.Rotate.down.add(
      "pointerDown",
      this.mouseDown
    );
    t.runScene.optionsEx.cb.events.Rotate.up.add("pointerUp", this.mouseUp);
  ...
  dispose() {
    dom.removeEventListener("Rotate", this.mouseDown);
    dom.removeEventListener("Rotate", this.mouseUp);
